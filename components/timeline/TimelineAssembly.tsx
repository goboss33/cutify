...
export function TimelineAssembly() {
    ...
    return (
        <section className={cn(
            "flex flex-col bg-[#0a0a0a] text-xs select-none border-t border-border transition-all duration-300 ease-in-out",
            isCollapsed ? "h-10" : "h-64"
        )}>
            {/* Timeline Header */}
            <div className="h-10 border-b border-white/10 flex items-center justify-between px-4 bg-neutral-900 shrink-0">
            ...
            {!isCollapsed && (
                <div className="flex-1 overflow-y-auto overflow-x-hidden relative animate-in fade-in slide-in-from-bottom-2 duration-300">
                    {/* Ruler */}
